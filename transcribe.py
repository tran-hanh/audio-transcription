...
# Configuration
CHUNK_LENGTH_MINUTES = 12  # Chunk size in minutes (adjust based on API limits)
...
def chunk_audio(audio_path: str, chunk_length_ms: int = CHUNK_LENGTH_MS) -> List[str]:
    """
    ...
    print(f"Splitting into approximately {num_chunks} chunks of ~{CHUNK_LENGTH_MINUTES} minutes each...")
...
def transcribe_chunk(model: genai.GenerativeModel, chunk_path: str, chunk_num: int, total_chunks: int) -> str:
    """
    ...
    try:
        ...
        # Wait for the file to be processed (Gemini needs to process uploaded files)
        ...
        prompt = f"Please transcribe this audio file. The primary language is Vietnamese ({LANGUAGE}), but there may be some English words mixed in. Provide only the transcription text, no additional commentary."
...
def transcribe_audio(
    ...
    Args:
        ...
        output_path: Path to save the final transcript (default: input_path + "_transcript.txt")
...
Examples:
  ...
  python3 transcribe.py input.mp3 --chunk-length 15
    ...
    parser.add_argument(
        "--api-key",
        help="Google Gemini API key (default: uses GEMINI_API_KEY environment variable)"
    )
